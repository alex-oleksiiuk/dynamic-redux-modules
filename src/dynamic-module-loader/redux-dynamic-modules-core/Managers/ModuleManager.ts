...
export function getModuleManager<State = {}>(
  ...
  const _seedReducers = () => {
    (_dispatch as Dispatch<AnyAction>)({ type: '@@Internal/ModuleManager/SeedReducers' });
  };
  ...
  const _addReducers = (reducerMap: ReducersMapObject<Reducer, AnyAction>) => {
    ...
    if (!_reducerManager) {
      ...
      for (const key in reducerMap) {
        _reducerManager.add(key, reducerMap[key as keyof typeof reducerMap]);
      }
    }
  };
  ...
  const moduleManager = {
    getReducer: _reduce as (s: State, a: AnyAction) => State,
    ...
    add: (modulesToAdd: IModule<any>[]) => {
      ...
      modulesToAdd.forEach(module => {
        if (!_moduleIds.has(module.id)) {
          ...
          _addReducers(module.reducerMap as ReducersMapObject<Reducer, AnyAction>);
      ...
      justAddedModules.forEach(module => {
        ...
        _dispatchActions(
          module.initialActions ? [moduleAddedAction, ...module.initialActions] : [moduleAddedAction]
        );
      });
    },
    remove: (modulesToRemove: IModule<any>[]) => {
      ...
      modulesToRemove.forEach(module => {
        if (_moduleIds.has(module.id)) {
          _dispatchActions(module.finalActions as AnyAction[]);

          _removeReducers(module.reducerMap as ReducersMapObject<Reducer, AnyAction>);
          _removeMiddlewares(module.middlewares as Middleware[]);

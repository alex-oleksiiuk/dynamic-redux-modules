...
import { IExtension, IItemManager, IModule, IModuleStore, IModuleTuple } from './Contracts';
...
export function createStore<State>(
  ...
  const extensionMiddleware = extensions.reduce((mw: any[], p) => {
    if (p.middleware) {
  ...
  const middlewareManager = getRefCountedManager(
    getMiddlewareManager() as IItemManager<unknown>,
    (a, b) => a === b
  ) as IItemManager<unknown> & { enhancer: any };
  ...
  const moduleManager = getRefCountedManager(
    getModuleManager<State>(
      middlewareManager as IItemManager<Middleware<unknown, any, Dispatch<AnyAction>>>,
      extensions,
      advancedCombineReducers
    ),
  ...
  // Create store
  const store: IModuleStore<State> = createReduxStore<State, Action<any>, unknown, unknown>(
    moduleManager.getReducer as unknown as Reducer<State, Action<any>>,
